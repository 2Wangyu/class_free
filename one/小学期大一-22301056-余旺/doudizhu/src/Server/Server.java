...
import java.util.ArrayList;
 ...
 //服务器主线程,用于服务器启动和接受玩家接入
public class Server {

    public static Room[] rooms = new Room[12];
    public static void main(String[] args) throws IOException {
        ServerSocket serverSocket = new ServerSocket(57498);//服务器端口
        Socket client;
        for (int i = 0; i < 12; i++) {
            rooms[i] = new Room();
        }
    ...
    public void run() {
        try {
            ...
            if (Room.n == 0){
                //由一个人的线程启动游戏线程
                Room.n = 3;
                new GameThread(room.getPlayers()).start();
                //开始游戏
            }
        ...
        while (true){
            ...
            if (!message.equals("-1")){
                ...
                if (rooms[choice].getNum() >= 3){
                    ...
                    //表示可以进入此房间
                    player.receiveMsg();
                    rooms[choice].addPlayer(player);
                    //房间记录玩家
                    player.setRoom(rooms[choice]);
                    //记录玩家的房间
                    return rooms[choice];
                }
            }
